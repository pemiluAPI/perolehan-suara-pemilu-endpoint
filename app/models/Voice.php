...
class Voice extends \Eloquent{
    ...
    {
        $id_of_propinsi = Wilayah::select('province_id')->where('id', '=', $id_of_wilayah)->get()->first();

        if (!empty($id_of_propinsi))
        {
            $id_of_propinsi = $id_of_propinsi->toArray();
            $id_of_propinsi = $id_of_propinsi['province_id'];
        }
        else
        {
            return false;
        ...
        return $id_of_propinsi;
    }

    private function getOneOrAllWilayahes($id_of_propinsi, $id_of_wilayah)
    {
        ...
        {
            $provinces_wilayahes = Wilayah::select('wilayahes.id', 'wilayahes.name')
                ->where('wilayahes.province_id', $id_of_propinsi)
                ->leftJoin('provinces', 'province_id', '=', 'provinces.id')
                // ->toSql();
                ->get()
                ->toArray();
    ...
    public function allVoicePaged($limit, $offset, $params=array())
    {
        ...
        // Get province first
        if (!$id_of_propinsi && $id_of_wilayah)
        {
            $id_of_propinsi = $this->getProvinceOfWilayah($id_of_wilayah);
            if (!$id_of_propinsi)
            {
        ...
        if (($id_of_pemilu > 0) && ($id_of_formulir > 0))
        {
            ...
            $provinces = Province::where('id', $id_of_propinsi)->get()->first();
            ...
            $provinces_wilayahes = $this->getOneOrAllWilayahes($id_of_propinsi, $id_of_wilayah);
                ...
                foreach ($provinces_wilayahes as $k_wilayah => $wilayah) {
                    ...
                    foreach ($formulirCategories as $k_formulirCategory => $formulirCategory) {
                        $voice_of_category = Voice::where('category_id', $formulirCategory['id'])
                            ->where('province_id', $id_of_propinsi)
